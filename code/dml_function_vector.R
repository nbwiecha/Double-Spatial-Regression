# Generic DML Functions for Partially Linear Model
...
# Note the internal syntax of these functions (lhat, ghat, psi_a, psi_b, etc.) 
# reflects notation in Chernozhukov (2018)'s double machine learning (DML) method
# and is less interpretable than notation used in our paper.

# The primary differences between these functions and those of the DoubleML
# package's are the ability to use a vector treatment variable as opposed to 
# scalar, emphasis on the GpGp package for fast Gaussian Process regression and 
# predictions (Kriging), and ability to more easily incorporate covariates
# as linear parts of the predictive models for treatment/response.

# binary_D indicates if treatments are binary, in which case GP
# predictions are truncated at 0, 1

################################################################################
#                     Primary DSR estimator - uses GpGp only                   #
################################################################################

dml_fit <- function(Y, D, S, X, K, refit=FALSE, covfun_name="matern_isotropic"){
  
  # if refit=FALSE, find LMM parameters only once for each Y/D var. 
  # if refit=TRUE, find LMM parameters for each fold. 
  ...
  # the following ensures all folds are of equal size if n/K is an integer
  ...
  for(k in 1:K){
    trainS <- S[folds!=k,,drop=FALSE]
    trainD <- D[folds!=k,,drop=FALSE]
    trainX <- X[folds!=k,,drop=FALSE]
    trainY <- Y[folds!=k,drop=FALSE]
    testS <- S[folds==k,,drop=FALSE]
    testX <- X[folds==k,,drop=FALSE]
    
    if(refit){
      
      fitY.k <- fit_model(y=trainY, locs=trainS, X=cbind(1, trainD, trainX), silent = TRUE,
                          covfun_name=covfun_name)
      predictionsY[[k]] <- predictions(fitY.k, locs_pred=testS, 
                                       X_pred=cbind(1, matrix(0, nrow=nrow(testS), ncol=p), testX))
      
      predD.k <- matrix(nrow=nrow(testS), ncol=p)
      for(j in 1:p){
        fitD.k <- fit_model(y=trainD[,j], locs=trainS, X=cbind(rep(1, nrow(trainS)), trainX), silent=TRUE,
                            covfun_name=covfun_name)
        predD.k[,j] <- predictions(fitD.k, locs_pred=testS,
...
################################################################################
#                   DML function for "theoretical DSR" if used with            #
#                predict_fn=svm_predict() from predict_functions_hpc.R         #
#             This ignores the treatment variable in the first-stage           #
#                        regression of Y onto S                                #
################################################################################
...
################################################################################
#                Similar function that just includes covariates in             #
#                        predictive model                                      #
################################################################################
